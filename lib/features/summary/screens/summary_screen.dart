          ...
          data: (trades) {
            final closed =
                trades.where((t) => t.status == TradeStatus.closed).toList()
                  ..sort(
                    (a, b) => (a.closedAt ?? a.openedAt).compareTo(
                      b.closedAt ?? b.openedAt,
                    ),
                  );
            final open = trades
...
class _Stats {
  ...
  double get capitalDeployed => closed.fold(0.0, (s, t) => s + t.costBasis);
  ...
  _TypeStats callStats() =>
      _TypeStats(closed.where((t) => t.optionType == OptionType.call).toList());
  _TypeStats putStats() =>
      _TypeStats(closed.where((t) => t.optionType == OptionType.put).toList());
  _TypeStats entryStats(EntryPointType type) =>
      _TypeStats(closed.where((t) => t.entryPointType == type).toList());
}
...
class _TypeStats {
  ...
  double get totalPnl => trades.fold(0.0, (s, t) => s + (t.realizedPnl ?? 0));
  double get avgWin {
...
class _Body extends ConsumerWidget {
            ...
            child: Column(
              children: [
                ...
                Text(
                  ...
                  style: TextStyle(color: AppTheme.neutralColor, height: 1.5),
                ),
              ],
            ),
          ),
  ...
  static Widget _emptyCard(String text) => Container(
    ...
    child: Center(
      child: Text(text, style: const TextStyle(color: AppTheme.neutralColor)),
    ),
  );
}
...
class _HeroCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final pnlColor = pnl >= 0 ? AppTheme.profitColor : AppTheme.lossColor;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: pnlColor.withValues(alpha: 0.4), width: 1.5),
      ),
      child: Column(
...
class _TypeCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final pnlColor = stats.totalPnl >= 0
        ? AppTheme.profitColor
        : AppTheme.lossColor;
    final wrColor = stats.winRate >= 50
        ? AppTheme.profitColor
        : AppTheme.lossColor;
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 2),
          Text(
            'Avg \$${stats.avgWin.toStringAsFixed(0)}',
            style: const TextStyle(color: AppTheme.neutralColor, fontSize: 11),
          ),
          const SizedBox(height: 2),
          Text(
...
class _BlockCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final pnlColor = block.totalPnl >= 0
        ? AppTheme.profitColor
        : AppTheme.lossColor;
    final winColor = winPct >= 50 ? AppTheme.profitColor : AppTheme.lossColor;
    final winTrades = block.trades
        .where((t) => (t.realizedPnl ?? 0) > 0)
        .toList();
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: block.edgeWarning
            ...
            : Border.all(color: AppTheme.borderColor.withValues(alpha: 0.25)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
